...
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(path_objec_auto, null, 2))
...
let data_pathauto = JSON.parse(fs.readFileSync(PathAuto + sender.split("@")[0] + ".json"))
if (data_pathauto.session === "buatroom") {
if (chats.length === 0) return;
if (isNaN(chats)) return reply("Hanya angka!")
data_pathauto.data.penerima = Number(chats);
if (data_pathauto.data.penerima == sender.split('@')[0]) return reply('jangan nomor lu')
if (data_pathauto.data.penerima == botNumber.split('@')[0]) return reply('itu kan nomor bot')
...
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
...
let btn_room = [{ buttonId: `${prefix}buat_room_chat ${sender}|${data_pathauto.data.penerima}@s.whatsapp.net|${roomC}`, buttonText: { displayText: 'Terima️' }, type: 1 }]
var but_room = {
...
conn.sendMessage(`${data_pathauto.data.penerima}@s.whatsapp.net`, but_room)
}
}
...
let ii = await fetchJson(`https://danzzapi.xyz/api/shortlink/bitly?url=${data_pathauto.data.trannss}&apikey=danzz`)
...
if (chats.length === 0) return;
data_pathauto.data.trannss = (chats)
let ii = await fetchJson(`https://danzzapi.xyz/api/shortlink/tinyurl?url=${data_pathauto.data.trannss}&apikey=danzz`)
if (ii.status == false) return reply('url tidak valid\nsilahkan kirim yg benar.')
data_pathauto.session = "input_texttttranss";
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
reply(`*SHORTLINK*
...
if (chats.length === 0) return;
data_pathauto.data.trannss = (chats)
let ii = await fetchJson(`https://danzzapi.xyz/api/shortlink/cuttly?url=${data_pathauto.data.trannss}&apikey=danzz`)
if (ii.status == false) return reply('url tidak valid\nsilahkan kirim yg benar.')
data_pathauto.session = "input_texttttranss";
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
reply(`*SHORTLINK*
...
var en = await fetchJson(`https://api.popcat.xyz/translate?to=en&text=${data_pathauto.data.trannss}`) 
data_pathauto.session = "input_texttttranss";
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
reply(`*TRANSLATE*
...
var ytmp4 = await fetchJson(`https://saipulanuar.ga/api/download/ytmp4?url=${data_pathauto.data.url_nya}&apikey=jPHjZpQF`)
var xx = ytmp4.result
...
if (chats.length === 0) return;
data_pathauto.data.url_nya = (chats);
var ytmp3 = await fetchJson(`https://saipulanuar.ga/api/download/ytmp3?url=${data_pathauto.data.url_nya}&apikey=jPHjZpQF`)
var xx = ytmp3.result
...
var x = await fetchJson(`https://api.popcat.xyz/npm?q=${data_pathauto.data.id_nya}`)
...
var git = await fetchJson(`https://api.github.com/users/${data_pathauto.data.id_nya}`)
...
var xx_besar = await fetchJson(`https://api.nataganz.com/api/random/besarkecil?text=${data_pathauto.data.text_nya}&apikey=Pasha`)
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
reply(`*BESAR KECIL*
...
data_pathauto.session = "text_nya_cuy";
var xx_bilang = await fetchJson(`https://api.nataganz.com/api/random/bilangangka?text=${data_pathauto.data.text_nya}&apikey=Pasha`)
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
reply(`*BILANG ANGKA*
...
data_pathauto.data.text_nya = Number(chats);
data_pathauto.session = "text_nya_cuy";
var xx_angka = await fetchJson(`https://api.nataganz.com/api/random/balikangka?text=${data_pathauto.data.text_nya}&apikey=Pasha`)
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
reply(`*BALIK ANGKA*
...
data_pathauto.data.text_nya = (chats);
data_pathauto.session = "text_nya_cuy";
var xx_huruf = await fetchJson(`https://api.nataganz.com/api/random/balikhuruf?text=${data_pathauto.data.text_nya}&apikey=Pasha`)
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_pathauto, null, 3));
reply(`*BALIK HURUF*
...
axios.get(`https://${hostwhm}:2087/json-api/createacct?api.version=1&username=${usern}&contactemail=mrsabaody@gmail.com&plan=${pekeg}&domain=${domain}`, authWhm)
.then(response => {     
...
conn.sendMessage(from, { caption: cap, image: { url: feta.data.qris }, buttons: buto, footer: '© created by SkyFalls' })
}
console.log(feta)
}
break
...
if (!q) return reply('link githubnya mana?\n*Contoh:*\n#gitclone https://github.com/Skyfalls24/MenfessV1')
var linknya = q
...
.then(data => {
var opt = { packname: 'Gurbot MD', author: 'By Skyfalls' }
